...
def adp_decision(x, network_ebm, max_iter, mode, config):
    min_step_lr = 1.0e-5
    lr_min = 1.0e-3
    ...
    with torch.no_grad():
        ...
        grad_uq = network_ebm(x, labels) # Score estimation for uncertainty quantification
        score_norm = torch.norm(grad_uq.view(grad_uq.shape[0], -1), dim=1)
        #decision_adv = torch.where(score_norm>25.0, 1.0, config.purification.dec_level) Use this
        decision_adv = torch.where(score_norm>35.0, 1.0, config.purification.dec_level) # Temporary for CIFAR10-C
        # low score function norm: x_pur with low noise
        # high score functio norm (attacked): x_pur with high noise
        x_pur = torch.clamp(x + torch.randn_like(x)*decision_adv[:,None,None,None]*config.purification.rand_smoothing_level, 0.0, 1.0)
        #x_pur = torch.clamp(x + torch.randn_like(x)*(torch.abs(grad_uq)*config.purification.uq_level + config.purification.rand_smoothing_level), 0.0, 1.0)
