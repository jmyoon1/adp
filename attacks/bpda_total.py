...
# Attack input x
def bpda_total(x, y, network_ebm, network_clf, config):
    ...
    for i in range(config.attack.iter):
        ...
        score_diff = torch.zeros_like(x_temp).to(config.device.ebm_device)
        for j in range(config.attack.n_eot):
            ...
            score_diff += (x_temp_eot - x_temp).to(config.device.ebm_device)
