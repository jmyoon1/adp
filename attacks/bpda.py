...
# Attack input x
def bpda(x, y, network_ebm, network_clf, config):
    ...
    for i in range(config.attack.iter):
        ...
        for j in range(config.attack.n_eot):
            if config.purification.purify_method=="adp_multiple_noise":
                if config.attack.n_eot > 1:
                    self.config.purification.rand_smoothing_level = (self.config.purification.lowest_level*j+self.config.purification.highest_level*(self.config.purification.rand_smoothing_ensemble-1-j))/(config.attack.n_eot-1)
                else:
                    self.config.purification.rand_smoothing_level = (self.config.purification.lowest_level+self.config.purification.highest_level)/2
