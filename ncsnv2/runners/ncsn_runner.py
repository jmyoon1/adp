...
__all__ = ['NCSNRunner', 'get_model']


def get_model(config):
    if config.data.dataset == 'CIFAR10' or config.data.dataset == 'CELEBA' or config.data.dataset=="TinyImageNet" or config.data.dataset == "CIFAR100":
...
class NCSNRunner():
    ...
    def train(self):
        ...
        if self.config.data.dataset=="TinyImageNet":
            nData=100000
        else:
            nData=50000
        indices = list(range(nData))
        train_sampler = SubsetRandomSampler(indices[:int(nData*0.9)])
        valid_sampler = SubsetRandomSampler(indices[int(nData*0.9):])
        dataloader = DataLoader(dataset, batch_size=self.config.training.batch_size, sampler=train_sampler,
